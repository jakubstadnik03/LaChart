  ...
  useEffect(() => {
    ...
    const combinedData = testings.flatMap(testing => 
      testing.points.map(point => {
        ...
        const dataPoint = {
          ...point,
          metric: testing.sport === 'run' ? pace : (speed / testing.weight).toFixed(2),
          power: speed,
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="custom-tooltip" style={{ backgroundColor: '#fff', padding: '5px', border: '1px solid #ccc' }}>
          ...
          <p style={{ color: entry.color }}>{testings[0]?.sport === 'run' ? 'Pace (min/km)' : 'Watts/Kg'}: {entry.payload.metric}</p>
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', p: 3 }}>
      <ResponsiveContainer width="100%" height={400}>
        <BarChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          ...
          <XAxis dataKey="name" />
          <YAxis yAxisId="left" orientation="left" stroke="#8884d8" unit={testings[0]?.sport === 'run' ? 'min/km' : 'W/Kg'} />
          <YAxis yAxisId="right" orientation="right" stroke="#82ca9d" unit="mmol/L" />
          <Tooltip content={<CustomTooltip />} />
          <Legend />
          <Bar yAxisId="left" dataKey="metric" fill="#8884d8" name={testings[0]?.sport === 'run' ? 'Pace (min/km)' : 'Watts per Kg'} />
