...
const EditAthleteModal = ({ athlete, open, onClose, onSave }) => {
  ...
  const handleChange = (e) => {
    ...
    if (name.includes(".")) {
      ...
      e.target.name === "personalBests" ||
      e.target.name === "injuries"
    ) {
      setFormData({
        ...formData,
        [e.target.name]: value.split(",").map((item) => item.trim()),
      });
    } else {
            ...
            {Object.entries(athlete.bikeZones)
              ...
              .map(([zone, values]) => (
                <TextField
                  ...
                  name={`bikeZones.${zone}`}
                  value={
                    Array.isArray(formData.bikeZones[zone])
                      ? formData.bikeZones[zone].join(", ")
                      : ""
                  }
                  onChange={handleChange}
                  margin="normal"
            ...
            {Object.entries(athlete.runZones)
              ...
              .map(([zone, values]) => (
                <TextField
                  ...
                  name={`runZones.${zone}`}
                  value={
                    Array.isArray(formData.runZones[zone])
                      ? formData.runZones[zone].join(", ")
                      : ""
                  }
                  onChange={handleChange}
                  margin="normal"
