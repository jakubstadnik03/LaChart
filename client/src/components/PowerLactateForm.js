...
import { ScatterChart, Scatter, XAxis, YAxis, CartesianGrid, Tooltip, Legend, LineChart, Line, ResponsiveContainer } from 'recharts';
import { TextField, Paper, Button, Box, Typography, IconButton, MenuItem, FormControl, InputLabel, Select } from '@mui/material';
import AddCircleOutlineIcon from '@mui/icons-material/AddCircleOutline';
import DeleteIcon from '@mui/icons-material/Delete';

const PowerLactateForm = ({ onSaveNewTesting }) => {
  const [inputFields, setInputFields] = useState([{ powerOrPace: '', lactate: '', heartRate: '' }]);
  ...
  const handleSaveNewTesting = () => {
    ...
    poolLength,
    weight,
      points: chartData.filter(field => field.powerOrPace && field.lactate && field.heartRate).map(field => ({
...
console.log(newTesting);
  ...
  const addInputField = () => {
    setInputFields([...inputFields, { powerOrPace: '', lactate: '', heartRate: '' }]);
  };
...
/>
      ...
      <Scatter name="Entries" data={chartData} fill="#8884d8">
        {
          chartData.map((entry, index) => (
            <circle key={`circle-${index}`} cx={entry.powerOrPace} cy={entry.lactate} r={4} stroke="#8884d8" fill="#8884d8" />
          ))
        }
      </Scatter>
  ...
  <div>
        ...
        <Box key={index} sx={{ display: 'flex', gap: 2, alignItems: 'center', mb: 2 }}>
          <TextField
            label={sport === 'swim' ? `Pace ${index + 1} (mm:ss/100m)` : sport === 'run' ? `Pace ${index + 1} (mm:ss/km)` : `Power ${index + 1}`}
            variant="outlined"
            value={input.powerOrPace}
            onChange={(e) => handleInputChange(index, 'powerOrPace', e.target.value)}
            fullWidth
          />
          <TextField
            ...
            variant="outlined"
            value={input.lactate}
            onChange={(e) => handleInputChange(index, 'lactate', e.target.value)}
            type="number"
          />
          <TextField
            ...
            onChange={(e) => handleInputChange(index, 'heartRate', e.target.value)}
            type="number"
          />
...
<TextField
  ...
  {sport === 'bike' && (
    <>
      <FormControl fullWidth sx={{ mb: 2 }}>
        <InputLabel id="indoor-outdoor-select-label">Indoor/Outdoor</InputLabel>
        <Select
  ...
  {sport === 'run' && (
    <>
      <FormControl fullWidth sx={{ mb: 2 }}>
        <InputLabel id="run-indoor-outdoor-select-label">Indoor/Outdoor</InputLabel>
        <Select
